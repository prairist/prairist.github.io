...
---`;
      ...
      if (!matterData.hasOwnProperty('permalink')) { // 永久链接
        if (matterData.editLink != false) {
          if (/[\\\/]_posts[\\\/]/.test(file.filePath)) {
            matterData.permalink = getPermalink('/posts/');
          } else if (/[\\\/](navi|food)[\\\/]/.test(file.filePath)) {
            matterData.permalink = getPermalink('/local/');
          } else if (/[\\\/]wiki[\\\/]/.test(file.filePath)) {
            matterData.permalink = getPermalink('/wiki/');
          } else {
            matterData.permalink = getPermalink('/pages/');
          }
          mark = true;
        }
      }
...
// 定义永久链接数据
function getPermalink(prefix) {
  return `${prefix + (Math.random() + Math.random()).toString(16).slice(2, 10)}/`
}
